...
void setup() {
  ...
  wificomponent->set_use_address("controller_1.local");
  ...
  mqtt_mqttclientcomponent->set_topic_prefix("devices/controller_1");
  WebServer *webserver = App.init_web_server(80);
  sensor::UptimeSensor *sensor_uptimesensor = App.make_uptime_sensor("Uptime Sensor controller_1");
  sensor::WiFiSignalSensor *sensor_wifisignalsensor = App.make_wifi_signal_sensor("WiFi Signal Sensor controller_1", 10000);
  switch_::RestartSwitch *switch__restartswitch = App.make_restart_switch("Restart controller controller_1");
  ...
  sensor::UltrasonicSensorComponent *sensor_ultrasonicsensorcomponent = App.make_ultrasonic_sensor("Niveau Tank 1", 12, 13, 2000);
  switch_::OutputSwitch *switch__outputswitch = App.make_output_switch("Pomp 1", pomp_uitvoer);
  switch_::OutputSwitch *switch__outputswitch_2 = App.make_output_switch("Klep 1", klep_uitvoer);
