...
void setup() {
  ...
  wificomponent->set_use_address("controller_4.local");
  ...
  mqtt_mqttclientcomponent->set_topic_prefix("devices/controller_4");
  WebServer *webserver = App.init_web_server(80);
  sensor::UptimeSensor *sensor_uptimesensor = App.make_uptime_sensor("Uptime Sensor controller_4");
  sensor::WiFiSignalSensor *sensor_wifisignalsensor = App.make_wifi_signal_sensor("WiFi Signal Sensor controller_4", 10000);
  switch_::RestartSwitch *switch__restartswitch = App.make_restart_switch("Restart controller controller_4");
  StatusLEDComponent *statusledcomponent = App.make_status_led(GPIOOutputPin(2, OUTPUT, false));
  sensor::PulseCounterSensorComponent *sensor_pulsecountersensorcomponent = App.make_pulse_counter_sensor("Flow door sensor controller_4", 4, 2000);
