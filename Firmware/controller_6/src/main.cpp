...
void setup() {
  ...
  wificomponent->set_use_address("controller_6.local");
  ...
  mqtt_mqttclientcomponent->set_topic_prefix("devices/controller_6");
  WebServer *webserver = App.init_web_server(80);
  sensor::UptimeSensor *sensor_uptimesensor = App.make_uptime_sensor("Uptime Sensor controller_6");
  sensor::WiFiSignalSensor *sensor_wifisignalsensor = App.make_wifi_signal_sensor("WiFi Signal Sensor controller_6", 10000);
  switch_::RestartSwitch *switch__restartswitch = App.make_restart_switch("Restart controller controller_6");
  StatusLEDComponent *statusledcomponent = App.make_status_led(GPIOOutputPin(2, OUTPUT, false));
  sensor::PulseCounterSensorComponent *sensor_pulsecountersensorcomponent = App.make_pulse_counter_sensor("Flow door sensor controller_6", 4, 2000);
